...
export default function Home() {
  ...
  const cartItems = cart.map((id) => products.find((item) => item.id === id)!).filter(Boolean);
  const total = cartItems.reduce((sum, item) => sum + item.price, 0);

  function addToCart(id: number) {
    setCart((current) => [...current, id]);
    setNotice("Товар добавлен в заказ");
  ...
  function sendOrder() {
    const lines = cartItems.map((item) => `${item.brand} ${item.model} — ${price(item.price)}`);
    const body = ["Заказ с сайта Уплотнитель23", ...lines, `Итого: ${price(total)}`, "", "Мой телефон: "].join("\n");
    window.location.href = `mailto:zapthasti2019@mail.ru?subject=${encodeURIComponent("Заказ уплотнителей")}&body=${encodeURIComponent(body)}`;
  }
  ...
  return <main>
    ...
    <SiteHeader actions={<button className="cart-button" onClick={() => setCartOpen(true)} aria-label={`Корзина, товаров: ${cart.length}`}><span aria-hidden="true">▱</span><b>{cart.length}</b></button>} />
    ...
    {cartOpen && <div className="drawer-backdrop" onMouseDown={() => setCartOpen(false)}><aside className="cart-drawer" onMouseDown={(e) => e.stopPropagation()} aria-label="Ваш заказ"><button className="drawer-close" onClick={() => setCartOpen(false)}>×</button><p className="eyebrow"><span /> Ваш заказ</p><h2>{cartItems.length ? `${cartItems.length} товар${cartItems.length > 1 ? "а" : ""}` : "Корзина пуста"}</h2>{cartItems.map((item, index) => <div className="cart-item" key={`${item.id}-${index}`}><div className="cart-thumb"><span className="mini-profile" /></div><div><b>{item.brand} {item.model}</b><span>{price(item.price)}</span></div><button onClick={() => setCart((current) => current.filter((_, i) => i !== index))}>×</button></div>)}{cartItems.length > 0 && <><div className="cart-total"><span>Итого</span><b>{price(total)}</b></div><button className="order-button" onClick={sendOrder}>Оформить по e-mail →</button></>}</aside></div>}
  </main>;
}
